...
const uint32_t LEAF_NODE_CELL_SIZE = LEAF_NODE_KEY_SIZE + LEAF_NODE_VALUE_SIZE; // cell = key + value
const uint32_t LEAF_NODE_SPACE_FOR_CELLS = PAGE_SIZE - LEAF_NODE_HEADER_SIZE;
const uint32_t LEAF_NODE_MAX_CELLS = LEAF_NODE_SPACE_FOR_CELLS / LEAF_NODE_CELL_SIZE; // 每页最多存放cells的数量
...
void create_new_root(Table* table, uint32_t right_child_page_num) {
     ...
     * Old root copied to new page, became left child
...
void leaf_node_split_and_insert(Cursor* cursor, uint32_t key, Row* value) {
    /*
     * Create the new node and move half the cells over.
     * Insert the new value in one of the wo nodes.
    ...
    /*
     * All existing keys plus new key should be divieded
